...
def build_dataset(
    X, y, train_mask, val_mask, test_mask, cat_features_idx,
    transformations: Transformations,
    cache = None
) -> Dataset:
    
    if cat_features_idx is not None:
        cat_features = X.columns[cat_features_idx]
        cat = True
    else:
        cat_features = []
        cat = False
    ...
    X_cat_train = X[cat_features].iloc[train_mask].to_numpy()
    X_cat_val = X[cat_features].iloc[val_mask].to_numpy()
    X_cat_test = X[cat_features].iloc[test_mask].to_numpy()

    X_num_train = X[num_features].iloc[train_mask].to_numpy()
    X_num_val = X[num_features].iloc[val_mask].to_numpy()
    X_num_test = X[num_features].iloc[test_mask].to_numpy()

    y_train = y.iloc[train_mask].to_numpy().flatten().astype(int)
    y_val = y.iloc[val_mask].to_numpy().flatten().astype(int)
    y_test = y.iloc[test_mask].to_numpy().flatten().astype(int)
    ...
    def load(train, val, test) -> ArrayDict:
        return {
            x: cast(np.ndarray, item)  # type: ignore[code]
            for x, item in zip(['train', 'val', 'test'], [train, val, test])
        }

    dataset = Dataset(
        load(X_num_train, X_num_val, X_num_test),
        load(X_cat_train, X_cat_val, X_cat_test) if cat else None,
    ...
    return transform_dataset(dataset, transformations)
